...
func TestResolveConflictsSuccess(t *testing.T) {
	...
	task, err := s.CreateTask(ctx, "conflict resolve test", 5)
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveConflictsContainerError(t *testing.T) {
	...
	task, err := s.CreateTask(ctx, "conflict error test", 5)
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveConflictsIsError(t *testing.T) {
	...
	task, err := s.CreateTask(ctx, "conflict is_error test", 5)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPruneOrphanedWorktrees(t *testing.T) {
	...
	task, err := s.CreateTask(ctx, "known task", 5)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunContainerSuccess(t *testing.T) {
	...
	out, stdout, stderr, err := r.runContainer(context.Background(), uuid.New(), "prompt", "", nil)
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
...
func TestRunContainerNonZeroExitWithValidOutput(t *testing.T) {
	...
	out, _, _, err := r.runContainer(context.Background(), uuid.New(), "prompt", "", nil)
	if err != nil {
...
func TestRunContainerEmptyOutputNonZeroExit(t *testing.T) {
	...
	_, _, _, err := r.runContainer(context.Background(), uuid.New(), "prompt", "", nil)
	if err == nil {
...
func TestRunContainerWithSessionID(t *testing.T) {
	...
	out, _, _, err := r.runContainer(context.Background(), uuid.New(), "prompt", "sess-xyz", nil)
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
	if out.StopReason != "end_turn" {
...
func TestBuildContainerArgsWithSessionID(t *testing.T) {
	r := newTestRunnerWithInstructions(t, "")
	args := r.buildContainerArgs("name", "prompt", "sess-abc", nil)
...
func TestBuildContainerArgsWorktreeOverride(t *testing.T) {
	...
	args := r.buildContainerArgs("name", "prompt", "", map[string]string{ws: wt})
...
func TestGenerateTitleSuccess(t *testing.T) {
	...
	task, err := s.CreateTask(ctx, "Fix the login bug in the authentication module", 5)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerateTitleSkipsExistingTitle(t *testing.T) {
	...
	task, err := s.CreateTask(ctx, "test prompt", 5)
	if err != nil {
		t.Fatal(err)
	}
...
{"result":"Add Auth Feature","session_id":"s1","stop_reason":"end_turn","is_error":false}`
	...
	task, err := s.CreateTask(ctx, "add authentication feature", 5)
	if err != nil {
		t.Fatal(err)
	}
...
// TestWriteProgressMDCreatesFile verifies that writeProgressMD creates
// PROGRESS.md in the workspace root when it doesn't exist yet.
func TestWriteProgressMDCreatesFile(t *testing.T) {
	repo := setupTestRepo(t)
	s, runner := setupTestRunner(t, []string{repo})
	ctx := context.Background()

	task, err := s.CreateTask(ctx, "Progress test task", 5)
	if err != nil {
		t.Fatal(err)
	}
	...
	updated, _ := s.GetTask(ctx, task.ID)
	if err := runner.writeProgressMD(updated, map[string]string{}); err != nil {
		t.Fatalf("writeProgressMD should not fail: %v", err)
	}

	progressPath := filepath.Join(repo, "PROGRESS.md")
	if _, err := os.Stat(progressPath); err != nil {
		t.Fatal("PROGRESS.md should be created:", err)
	}
	...
	if !strings.Contains(string(content), "Progress Log") {
		t.Fatalf("PROGRESS.md should contain header; got:\n%s", content)
	}
}

// TestWriteProgressMDAppendsToExisting verifies that writeProgressMD appends
// to an existing PROGRESS.md rather than overwriting it.
func TestWriteProgressMDAppendsToExisting(t *testing.T) {
	repo := setupTestRepo(t)
	s, runner := setupTestRunner(t, []string{repo})
	ctx := context.Background()
	...
	task, err := s.CreateTask(ctx, "Append test", 5)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(content), "existing content") {
		t.Fatal("existing content should still be present after append")
	}
	if !strings.Contains(string(content), "## Task:") {
		t.Fatal("new task entry should be appended")
	}
}
...
func TestRunContainerContextCancelled(t *testing.T) {
	...
	_, _, _, err := r.runContainer(ctx, uuid.New(), "prompt", "", nil)
	if err == nil {
